       ...
       01  WS-FORMATTED-NUMBER   PIC X(20) VALUE SPACES.
       ...
       PROCEDURE DIVISION USING LK-RAW-PHONE-NUMBER.
           ...
           *> Extract numeric digits from the input
           PERFORM UNTIL WS-INDEX > FUNCTION LENGTH(LK-RAW-PHONE-NUMBER)
               MOVE LK-RAW-PHONE-NUMBER(WS-INDEX:1) TO WS-CURRENT-CHAR
               IF WS-CURRENT-CHAR >= "0" AND WS-CURRENT-CHAR <= "9"
                   IF WS-DIGIT-COUNT < 10
                       MOVE WS-CURRENT-CHAR TO WS-CLEAN-NUMBER(WS-DIGIT-INDEX:1)
                       ADD 1 TO WS-DIGIT-COUNT
                       ADD 1 TO WS-DIGIT-INDEX
                   ELSE
                       ADD 1 TO WS-DIGIT-COUNT  *> Track overflow (ignore extra digits)
                   END-IF
               END-IF
               ADD 1 TO WS-INDEX
           END-PERFORM
