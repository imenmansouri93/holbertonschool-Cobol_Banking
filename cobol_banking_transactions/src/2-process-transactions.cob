           ...
           EVALUATE FUNCTION TRIM(ACTION)
               WHEN "INSERT"
                   *> Insert customer
                   ...
                   STRING "INSERT INTO customers(name) VALUES('" CUST-NAME "')" DELIMITED BY SIZE
                       INTO SQL-COMMAND
                   CALL STATIC "DB_EXECUTE" USING BY VALUE DBH, BY REFERENCE SQL-COMMAND RETURNING DUMMY-RC

                   *> Insert account
                   MOVE SPACES TO SQL-COMMAND
                   STRING "INSERT INTO accounts(customer_id, balance) VALUES((SELECT MAX(id) FROM customers), " AMOUNT ")"
               ...
               WHEN "DEPOSIT"
                   ...
                   STRING "UPDATE accounts SET balance = balance + " AMOUNT
                       " WHERE account_id = " ACCT-ID DELIMITED BY SIZE INTO SQL-COMMAND
                   CALL STATIC "DB_EXECUTE" USING BY VALUE DBH, BY REFERENCE SQL-COMMAND RETURNING DUMMY-RC
